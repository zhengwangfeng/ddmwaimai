...
public class Test {
	...
	public void hjDelShelf() {
		...
		JwShelfAPI.doDelShelfManager(newAccessToken,2);
	}
	...
	public void hjUpdateShelf() {
		...
		e.setEid(5);
		e.setImg_background(JwProductAPI.uploadImg(newAccessToken,filePath, fileName));
		...
		shelf.setShelf_banner(JwProductAPI.uploadImg(newAccessToken,filePath, fileName));
		shelf.setShelf_name("测试货架");
		shelf.setShelf_id(2);
		JwShelfAPI.doUpdateExpress(newAccessToken,shelf);
	}
	
	// 获取所有货架信息
	public void getAllShelf() {
		JwShelfAPI wss = new JwShelfAPI();
		JwShelfAPI.getAllShelf(newAccessToken);
	}
	...
	public void getByShelfId() {
		JwShelfAPI wss = new JwShelfAPI();
		JwShelfAPI.getByShelfId(newAccessToken,4);
	}
	...
	public void hjAddTest1() {
		...
		shelf.setShelf_banner(JwProductAPI.uploadImg(newAccessToken,filePath, fileName));
		shelf.setShelf_name("测试货架");
		JwShelfAPI.doAddExpress(newAccessToken,shelf);
	}
	...
	public void hjAddTest2() {
		...
		group_info.setImg(JwProductAPI.uploadImg(newAccessToken,filePath, fileName));
	...
	public void ddGetidTest() {
		...
		JwOrderManagerAPI.getByOrderId(newAccessToken,"7197417460812533543");
	}
	...
	public void ddgetBystatusTest() {
		...
		JwOrderManagerAPI.getByFilter(newAccessToken,orderPara);
	}
	...
	public void fzAddStockTest() {
		...
		org.jeewx.api.wxstore.stock.JwStockAPI.doAddStock(newAccessToken,stockInfo);
	}
	...
	public void fzSubStockTest() {
		...
		org.jeewx.api.wxstore.stock.JwStockAPI.doSubStock(newAccessToken,stockInfo);
	}
	...
	public void fzDelTest() {
		...
		GroupRtnInfo gdis = JwGroupManangerAPI.doDelGroupManager(newAccessToken,205038402);
	}
	
	// 获取分组信息
	public void fzgetAllTest() {
		JwGroupManangerAPI wgms = new JwGroupManangerAPI();
		List<GroupDetailInfo> gdis = JwGroupManangerAPI.getAllGroup(newAccessToken);
	}
	...
	public void fzUpdageProductTest() {
		...
		GroupRtnInfo gdi = JwGroupManangerAPI.doUpdateGroupManagerProduct(newAccessToken,g);
	}
	...
	public void fzUpdagePropertisTest() {
		...
		GroupRtnInfo gdi = JwGroupManangerAPI.doUpdateGroupManagerProperties(newAccessToken,g);
	}
	
	// 根据分组ID获取分组信息
	public void fzgetByidTest() {
		JwGroupManangerAPI wgms = new JwGroupManangerAPI();
		GroupDetailInfo gdi = JwGroupManangerAPI.getByGroupId(newAccessToken,205038402);
	}
	...
	public void fzAddTest() {
		...
		GroupRtnInfo r = JwGroupManangerAPI.doAddGroupManager(newAccessToken,group);
	}
    ...
    public void spAddTest() {
    	...
    	attrInfo.setMain_img(JwProductAPI.uploadImg(newAccessToken,filePath, fileName));
    	List<String> imgs = new ArrayList<String>();
    	imgs.add(JwProductAPI.uploadImg(newAccessToken,filePath, fileName));
    	attrInfo.setImg(imgs);
    	// 商品详情列表
    	List<AttrInfoDetail> details = new ArrayList<AttrInfoDetail>();
    	AttrInfoDetail detail1 = new AttrInfoDetail();
    	detail1.setText("苹果");
    	detail1.setImg(JwProductAPI.uploadImg(newAccessToken,filePath, fileName));
    	details.add(detail1);
    	AttrInfoDetail detail2 = new AttrInfoDetail();
    	detail2.setText("橘子");
    	detail2.setImg(JwProductAPI.uploadImg(newAccessToken,filePath, fileName));
    	details.add(detail2);
    	...
    	CommodityRtnInfo object = JwProductAPI.doAddCommodity(newAccessToken,product);
    }
	
	// 获取指定ID的邮费模板
	public void getAllTest() {
		JwDeliveryMoneyAPI wps = new JwDeliveryMoneyAPI();
		List<DeliveryMoney> p = JwDeliveryMoneyAPI.getAllExpress(newAccessToken);
	...
	public void getMbTest() {
		JwDeliveryMoneyAPI wps = new JwDeliveryMoneyAPI();
		DeliveryMoney p = JwDeliveryMoneyAPI.getByIdExpress(newAccessToken,205036446);
		System.out.println(p.getTemplate_id());
	}
	...
	public void yfUpdateTest() {
		...
		topFee.add(ptf);
		p.setTopFee(topFee);
		DeliveryMoneyRtnInfo rtn = JwDeliveryMoneyAPI.doUpdateExpress(newAccessToken,p);
	...
	public void yfAddTest() {
		...
		topFee.add(ptf);
		p.setTopFee(topFee);
		DeliveryMoneyRtnInfo rtn = JwDeliveryMoneyAPI.doAddExpress(newAccessToken, p);
	}
}
