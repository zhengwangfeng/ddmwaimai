...
<html>
 ...
 <body>
		<t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="table" action="tcsbMembershipLevelController.do?doUpdate" >
		...
		<table style="width: 600px;" cellpadding="0" cellspacing="1" class="formtable">
					<tr>
						...
						<td class="value">
						     	 <input id="name" name="name" type="text" style="width: 150px" class="inputxt"  value='${tcsbMembershipLevelPage.name}' datatype="*">
				...
				<td class="value">
					...
					<select id="memberLevelConditionsId" name="memberLevelConditionsId" datatype="*">
						...
						<c:forEach items="${tcsbMemberLevelConditionsEntitys}" var="tcsbMemberLevelConditionsEntity">
							<option value="${tcsbMemberLevelConditionsEntity.id}"  <c:if test="${tcsbMembershipLevelPage.memberLevelConditionsId==tcsbMemberLevelConditionsEntity.id}">selected ="selected"</c:if>>${tcsbMemberLevelConditionsEntity.charge}</option>
						</c:forEach>
					</select>
					<span class="Validform_checktip"></span>
