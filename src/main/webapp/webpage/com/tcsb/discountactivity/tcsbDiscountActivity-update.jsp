...
<html>
 ...
 <body>
		<t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="table" action="tcsbDiscountActivityController.do?doUpdate" tiptype="1" >
						...
						<td class="value">
							...
							<select id="fullcutTemplateId" name="fullcutTemplateId">
								...
								<c:forEach items="${tcsbFullcutTemplateEntities}" var="tcsbFullcutTemplateEntitie">
									<option value="${tcsbFullcutTemplateEntitie.id }" <c:if test="${tcsbFullcutTemplateEntitie.id==tcsbDiscountActivityPage.fullcutTemplateId}">selected="selected"</c:if>>满${tcsbFullcutTemplateEntitie.total}减${tcsbFullcutTemplateEntitie.discount}</option>
								</c:forEach> 
							</select>
