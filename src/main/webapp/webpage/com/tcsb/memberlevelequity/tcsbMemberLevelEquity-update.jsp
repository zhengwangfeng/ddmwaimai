...
<html>
 ...
 <body>
		<t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="table" action="tcsbMemberLevelEquityController.do?doUpdate"  >
				...
				<td class="value">
					...
					<select id="membershipLevelId" name="membershipLevelId" datatype="*">
						...
						<c:forEach items="${tcsbMembershipLevelEntitys}" var="tcsbMembershipLevelEntity">
							<option value="${tcsbMembershipLevelEntity.id}"  <c:if test="${tcsbMemberLevelEquityPage.membershipLevelId==tcsbMembershipLevelEntity.id}">selected ="selected"</c:if>>${tcsbMembershipLevelEntity.name}</option>
						</c:forEach>
					</select>
					...
					<tr>
						...
						<td class="value">
									<t:dictSelect field="isDiscount" type="select"
										typeGroupCode="sf_yn" defaultVal="${tcsbMemberLevelEquityPage.isDiscount}" hasLabel="false"  title="是否打折" datatype="*"></t:dictSelect>
							<span class="Validform_checktip"></span>
 ...
 <script>
     $(function(){
         var html="<tr id='tr1'>"+
             ...
             "<label class='Validform_label' style='display: none;'>打折</label>"+
             "</td>";
		 if($("select[name='isDiscount']").val()=="Y"){
